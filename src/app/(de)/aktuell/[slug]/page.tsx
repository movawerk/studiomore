...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  return buildMetadata({ title: t(item.title, "de"), description: t(item.intro, "de"), locale: "de", path: newsHref(item.slug, "de"), alternatePath: newsHref(item.slug, "en"), ogImage: item.image?.src });
}
export default async function Page({ params }: Props) {
  const item = getNewsItem((await params).slug);
  if (!item?.published) notFound();
  return <NewsDetailPage item={item} locale="de" />;
}
